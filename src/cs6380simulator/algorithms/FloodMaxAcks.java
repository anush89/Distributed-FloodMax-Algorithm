...
public class FloodMaxAcks extends Algorithm {
	...
	public FloodMaxAcks(int nodeId) {
		super(nodeId);
	...
	public void processMessages(HashMap<Integer, Link> incoming) {
		...
		if(parent_node==nodeId && done_status.isEmpty() && round_no > 0)
		{
		...
		if((child_nodes.isEmpty() || done_status.isEmpty()) && round_no > 0 && parent_node != nodeId && !leader_changed && isTerm==false)
		{
		...
		logMessage(String.format("Parent: %d, Tentative Leader: %d, Round_no: %d", parent_node, tentative_leader, round_no));
	}
	...
	private enum MessageType {
		EXPLORE, DONE, ACK, TERMINATE, PRUNE
	}
}